...
export const getVerifiedCandidates = catchAsync(async (req, res) => {
  const features = new APIFeatures(
    Candidate.find({ isVerified: { $eq: true } }),
    req.query
  )
    .filter()
    .limitingFields()
...
export const getOneCandidate = catchAsync(async (req, res, next) => {
  const candidate = await Candidate.findById(req.params.id);
  if (!candidate) return next(new APPError("user not found", 404));
  res.status(200).json({
    status: "success",
    candidate,
  });
});
export const getCurrentCandidate = catchAsync(async (req, res, next) => {
