...
export const getAllAdmins = catchAsync(async (req, res, next) => {
  const admin = await Voter.find({ role: { $ne: "voter" } });
  if (!admin) {
...
export const updatedVoterPassword = catchAsync(async (req, res, next) => {
  ...
  const updatedPassword = await Voter.findById(req.params.id);
  if (!updatedPassword) {
    return next(new APPError("user for this id is not found", 404));
  }
  updatedPassword.password = req.body.password;
  await updatedPassword.save();
  res.status(200).json({
    status: "success",
    message: "password changed successfully",
    updatedVoter: updatedPassword,
  });
});
...
export const getYourVote = catchAsync(async (req, res, next) => {
  const voterId = req.voter.id;
  const votes = await Vote.findOne({ voter: voterId })
    .populate({
      path: "candidate",
      select: "firstName lastName photo",
    })
    .select("-voter");
  // if (!votes) {
  //   return next(new APPError(`sorry! you didn't have vote yet!`, 400));
  // }
  res.status(200).json({
    status: "success",
    vote: votes,
