...
export const getUnverifiedVoter = catchAsync(async (req, res, next) => {
  const features = new APIFeatures(
    Voter.find({ role: { $eq: "voter" }, isVerified: { $ne: true } }),
    req.query
  )
    .filter()
    .limitingFields()
...
export const changeToAdmin = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json({
    status: "success",
    message: "the voter is now changed to an admin",
    voter,
  });
});
export const changeToVoter = catchAsync(async (req, res, next) => {
  const voter = await Voter.findByIdAndUpdate(
    req.params.id,
    { role: "voter" },
    { new: true, runValidators: true }
  ...
  res.status(200).json({
    status: "success",
    message: "the admin is now changed to a voter",
    voter,
  });
});
